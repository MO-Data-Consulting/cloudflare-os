...
import {
  ...
  extractRfc822Attachments, newGmailMessageId, normalizeAggregateRecipients,
...
async function parseSafeGmailDraft(message: GmailMessageRaw): Promise<GmailParsedDraft> {
  ...
  const parsedIndexes = parsed.attachments.flatMap((attachment, index) =>
    attachment.contentType.toLowerCase() === "message/rfc822" ? [index] : []);
  if (parsedIndexes.length === 0 && exactAttachments.length === 0) return parsed;
...
async function mapMatchingApplyingDraftCreate(
  ...
  if (await gmailDraftFingerprint(current, provider.message.threadId) !==
      await gmailDraftStateFingerprint(create.draft)) {
    throw new Error("A discovered Gmail draft has the approved Message-ID but different content.");
  }
  return store.mapDraftToProvider(resource.logicalId, provider.id, matched.id);
}
...
async function loadSimulatedDraft(
  ...
  const base = await loadDraftBase(ctx, resource, captureAttachments);
...
async function messageStillAvailable(
  ...
  const rfcMessageId = metadata.payload?.headers?.find(
    header => header.name.toLowerCase() === "message-id")?.value.trim();
  if (!rfcMessageId || !/^<[^<>\s@]+@[^<>\s@]+>$/.test(rfcMessageId)) return false;
  const query = combineGmailQueries(
    ctx.searchQuery, `rfc822msgid:${rfcMessageId.slice(1, -1)}`);
  let pageToken: string | undefined;
  do {
    const result = await ctx.api.listMessages(100, query, pageToken);
    if (result.messages.some(message => message.id === messageId)) return true;
...
async function restrictedThreadScope(
  ...
  for (let i = 0; i < messageIds.length; i += 5) {
    const checked = await Promise.all(messageIds.slice(i, i + 5).map(async messageId => {
      ...
      return await messageStillAvailable(ctx, messageId, metadata) ? messageId : undefined;
    }));
    for (const messageId of checked) {
      if (messageId !== undefined) admitted.push(messageId);
    }
  }
  if (!admitted.length) {
...
function gmailMessageCursor(
  ...
  return new RpcCursor(new CursorPager<{
    ...
    async buildEntries(messages) {
      const entries: GmailMessageEntry[] = [];
      for (let i = 0; i < messages.length; i += 5) {
        entries.push(...await Promise.all(messages.slice(i, i + 5).map(async ref => {
          ...
          return {info, message: new GmailMessageStub(ctx, ref.id, ref.threadId, scope)};
        })));
      }
      return entries;
    },
...
function gmailFullThreadCursor(
  ...
  return new RpcCursor(new CursorPager<{id: string; snippet?: string}, GmailThreadEntry>({
    provider: "Gmail",
    async fetchPage(pageToken) {
...
class GmailSessionImpl extends GmailRpcTarget implements GmailSession {
  ...
  async renameLabel(label: GmailCustomLabel, name: string): Promise<GmailCustomLabel> {
    ...
    await submitAction(this.#ctx, {
      type: "labelRename", labelId: resource.logicalId, name, dependsOn: dependencies,
    }, {
...
class GmailThreadStub extends GmailRpcTarget implements GmailThread {
  ...
  async #loadInfo(): Promise<GmailThreadInfo> {
    ...
    const metadata = await Promise.all(admitted.map(id => this.#ctx.api.getMessageMetadata(id)));
...
class GmailMessageStub extends GmailRpcTarget implements GmailMessage {
  ...
  async #info(): Promise<GmailMessageInfoRaw> {
    if (this.#cachedInfo) return this.#cachedInfo;
    if (this.#cachedRaw) {
      this.#cachedInfo = await this.#ctx.api.parseMessageInfo(this.#cachedRaw);
    } else {
      const metadata = await this.#ctx.api.getMessageMetadata(this.#messageId);
...
function gmailDraftCursor(ctx: GmailContext): Cursor<GmailDraftEntry> {
  ...
  return new RpcCursor(new CursorPager<DraftCursorRef, GmailDraftEntry>({
    provider: "Gmail",
    async fetchPage(pageToken) {
      ...
      for (const draft of page.drafts) {
        ...
        if (items.some(item => item.logicalId === resource.logicalId)) continue;
...
function restrictedDraftCursor(ctx: GmailContext): Cursor<GmailDraftEntry> {
  ...
  return new RpcCursor(new CursorPager<GmailDraftResource, GmailDraftEntry>({
    provider: "Gmail",
    async fetchPage(pageToken) {
...
class GmailDraftStub extends GmailRpcTarget implements GmailDraft {
  ...
  async attachments(): Promise<GmailAttachmentEntry[]> {
    ...
    let entries: GmailAttachmentEntry[];
  ...
  async update(patch: GmailDraftPatch): Promise<void> {
    ...
    await submitAction(this.#ctx, {
      ...
      description: describeDraftAction(
        "Replace the selected draft fields.", after,
        sourceSnapshot
  ...
  async send(): Promise<void> {
    ...
    await submitAction(this.#ctx, {
      ...
      ...(dependencies.length === 0 && state.messageId
        ? {expectedProviderMessageId: state.messageId}
        : {}),
      ...(state.rfcMessageId ? {messageId: state.rfcMessageId} : {}),
      ...(sourceSnapshot ? {sourceAttachment: sourceSnapshot} : {}),
      dependsOn: dependencies,
    }, {
      title: sanitizeApprovalTitle(`Send Gmail draft: ${state.subject || "(no subject)"}`),
      description: describeDraftAction("Send this exact draft snapshot.", state),
      awaitDecision: true,
    });
  }
}
...
async function applyMessageMutation(
    api: GmailApi, store: GmailStore, action: GmailMessageMutationAction): Promise<void> {
  ...
  if (action.target.kind === "thread") {
    if (action.operation === "trash" || (trashLabelOperation && action.operation === "applyLabel")) {
      await api.trashThread(action.target.threadId);
    } else if (trashLabelOperation) {
      await api.untrashThread(action.target.threadId);
    }
    else await api.modifyThread(action.target.threadId, labels!.add, labels!.remove);
    return;
  }
  for (const messageId of action.target.messageIds) {
    if (action.operation === "trash" || (trashLabelOperation && action.operation === "applyLabel")) {
      await api.trashMessage(messageId);
    } else if (trashLabelOperation) {
      await api.untrashMessage(messageId);
    }
    else await api.modifyMessage(messageId, labels!.add, labels!.remove);
  }
}
...
export class GmailGatekeeperImpl extends DurableObject<Env, GmailGatekeeperImplProps>
  ...
  async #getSelfEmail(): Promise<string> {
    ...
    const description = await getGoogleAccountDescription(await this.#getAccessToken());
    if (!description.uniqueName) throw new Error("Google account has no email address.");
    if (cached && cached.toLowerCase() !== description.uniqueName.toLowerCase()) {
      throw new Error(
        "This Gmail binding belongs to a different Google account. Reconnect the original account.");
    }
  ...
  async applyAction(actionId: number): Promise<void> {
    ...
    try {
      ...
      case "messageMutation":
        await applyMessageMutation(api, store, action);
        break;
      case "send": {
      ...
      case "draftDelete": {
        ...
        try {
          ...
          if (!(store.isApplying(actionId) && error instanceof GmailApiError && error.status === 404)) {
            throw error;
          }
          const resource = store.getDraft(action.draftId)!;
      ...
      case "labelCreate": {
        ...
        if (!created.id || created.type !== "user") throw new Error("Gmail returned an invalid label.");
        resource.providerId = created.id;
        resource.name = created.name;
      ...
      case "labelRename": {
        ...
        const renamed = await api.renameLabel(id, action.name);
