/**
 * A forward-only pagination cursor returned by Gmail list and search methods.
 *
 * This is an RPC object, not a JavaScript iterator: it has no `collect()` method or
 * async-iteration protocol, and `next()` returns the next batch directly rather than an
 * iterator result such as `{value, done}`. Call `next()` repeatedly on the same cursor until it
 * returns `null`, and dispose the cursor when finished.
 *
 * @example
 * ```ts
 * const cursor = await gmail.searchMessages("is:unread");
 * for (;;) {
 *   const batch = await cursor.next();
 *   if (batch === null) break;
 *   for (const entry of batch) {
 *     console.log(entry.info.subject);
 *   }
 * }
 * ```
 */
export interface Cursor<T> {
  /** The next complete batch of entries, or `null` once the cursor is exhausted. */
  next(): Promise<T[] | null>;
}
