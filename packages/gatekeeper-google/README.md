...
Account and shared-drive bindings also queue creation of blank native Google Docs, blank native Google Sheets, and folders. An omitted destination means the My Drive root for an account binding or the bound shared-drive root; callers may instead name an in-scope, writable folder created by this app. Exact-file bindings remain read-only, even when the selected file is a folder.

Google Doc creation returns a plain-data handle. A gadget can persist that handle between scheduled callbacks, call `openCreatedGoogleDoc()` to obtain a fresh writable child session, and dispose the child after each callback. Before provider creation is approved, content reads and Markdown edits replay against durable local simulation without a Docs API call. After approval, the same handle resolves to that exact app-created Doc and pending approved edits materialize once in order. Arbitrary Docs opened by file ID remain read-only.

Unattended scheduled creation and writing require the owner to enable both auto-approval rules: **Google Doc creation** and **Document edits**. If either rule is absent, the first manual gate prevents later actions from being skipped. Google Sheet and folder creation always remain manual.

The new item contains no initial content and inherits the destination folder's permissions. `getCreationResult()` reports `pending` (with `lastError` after a retryable failure), `rejected`, `created`, or `reverted`. Revert moves an item to Drive trash; it never permanently deletes it. Rejected and reverted handles cannot reopen a Doc.

Every native open and creation destination lookup re-fetches Drive metadata. Scope and returned-ID checks precede observation authorization; MIME type, trash state, app marker, and current-capability checks follow authorization. A folder, shortcut, non-native blob, wrong native type, or out-of-scope file cannot mint a content capability. The Drive API exposes no agent-authored raw `q` strings, generic upload or write primitive, shortcut traversal, arbitrary download or export, or Workers AI extraction. Direct Google Doc bindings retain their existing editing API; only Drive Docs opened through their authoritative creation handle gain editing.

Account bindings request `drive.metadata.readonly`, `documents.readonly`, `spreadsheets.readonly`, and `drive.file`. Shared-drive bindings request `drive.readonly` plus `drive.file`. The `drive.file` scope limits writes to files this app creates or the user explicitly opens with it; the gatekeeper further restricts creation to an authorized destination and edits to the exact marked Doc created by that request. Older account and shared-drive connections without `drive.file` must expand Google consent before using these writable workflows. Current grants already include it, so this workflow adds no further scope. Exact-file bindings remain on the three read-only scopes, and direct Google Doc and Spreadsheet grants are unchanged.
