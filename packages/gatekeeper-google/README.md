...
Each create call returns an asynchronous handle and submits a manual approval that names the destination. The new item contains no initial content and inherits the destination folder's permissions. `getCreationResult()` reports `pending` (with `lastError` after a retryable failure), `rejected`, `created`, or `reverted`. Revert moves an item to Drive trash; it never permanently deletes it.
