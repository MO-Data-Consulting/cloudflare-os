...
/** Whether failed trials should be reported without failing the process. */
export function evalReportOnly(environment: NodeJS.ProcessEnv = process.env): boolean {
  const value = environment.WORKSHOP_EVAL_REPORT_ONLY?.trim();
  if (value === undefined || value === "" || value === "false") return false;
  if (value === "true") return true;
  throw new Error("WORKSHOP_EVAL_REPORT_ONLY must be true or false");
}
