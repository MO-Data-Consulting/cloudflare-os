...
import { evalMatrix, evalReportOnly, resolveEvalCommit } from "./config.js";
...
const reportOnly = evalReportOnly();
...
export function defineTaskEval(task: EvalTask): void {
  ...
  describeEval(task.id, { harness }, it => {
    it.for(cases)("$name", async ({ model, trial }, { run }) => {
      ...
      await expect(result).toSatisfyJudge(FunctionalJudge, {
        threshold: reportOnly ? null : 1,
      });
    });
  });
}
