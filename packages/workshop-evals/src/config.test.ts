...
import {
  ...
  evalReportOnly, resolveEvalCommit,
} from "./config.js";
...
it("fails trials by default and accepts an explicit report-only mode", () => {
  expect(evalReportOnly({})).toBe(false);
  expect(evalReportOnly({ WORKSHOP_EVAL_REPORT_ONLY: "false" })).toBe(false);
  expect(evalReportOnly({ WORKSHOP_EVAL_REPORT_ONLY: "true" })).toBe(true);
  expect(() => evalReportOnly({ WORKSHOP_EVAL_REPORT_ONLY: "yes" })).toThrow("true or false");
});
