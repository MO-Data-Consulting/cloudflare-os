...
describe("accountCoversVendorResources", () => {
  it("detects a stored account capability that predates a new resource type", () => {
    const file = resource("https://data.example/files/:fileId", "Data File");
    const library = resource("https://data.example/library", "Data Library");
  ...
  it("uses the URL pattern as the stable resource-type identity", () => {
    const oldResource = resource("https://data.example/library", "Old title");
    const renamedResource = resource("https://data.example/library", "New title");
