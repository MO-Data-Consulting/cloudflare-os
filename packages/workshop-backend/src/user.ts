...
export class UserDurableObject extends DurableObject<Cloudflare.Env> {
  ...
  *#connectedAccountRecords(): Generator<ConnectedAccountRecord> {
  ...
  async #provisionMissingAccounts(): Promise<void> {
    ...
    const provisioned = new Map<string, ConnectedAccountRecord[]>();
    for (let rec of this.#connectedAccountRecords()) {
      if (!rec.autoProvisioned) continue;
      const records = provisioned.get(rec.vendorId) ?? [];
      records.push(rec);
    ...
    for (let {vendorId, vendor} of await this.#ambientVendors()) {
      ...
      const records = provisioned.get(vendorId) ?? [];
      if (records.length > 0) {
        try {
          const vendorResources = await vendor.getSupportedResources();
          const accountResourceSets = await Promise.all(
  ...
  async listProvidedAccounts(): Promise<ProvidedAccountInfo[]> {
    ...
    const ambientResultIndexes = new Map<string, number>();
    for (let rec of this.#connectedAccountRecords()) {
      ...
      const info = { accountId: rec.id, vendorId: rec.vendorId, description: rec.description };
      if (rec.autoProvisioned) {
        ...
        const previousIndex = ambientResultIndexes.get(rec.vendorId);
